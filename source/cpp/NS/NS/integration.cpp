...
void right_part(ConfigParam &cp, MainData &md, double * ks, double * x, double time, double impulse)
{
	...
	ks[2] = -(I_mem + I_th + I_syn) / cp.e_3_c;

	double alpha_n = cp.e_4_alpha_a1 * (cp.e_4_alpha_a2 - x[2]) / (exp((cp.e_4_alpha_a2 - x[2]) / cp.e_4_alpha_a3) - 1.0);
	double beta_n = cp.e_4_beta_b1 * exp(-x[2] / cp.e_4_beta_b2);
	ks[3] = alpha_n * (1.0 - x[3]) - beta_n * x[3];

	double alpha_m = cp.e_5_alpha_a1 * (cp.e_5_alpha_a2 - x[2]) / (exp((cp.e_5_alpha_a2 - x[2]) / cp.e_5_alpha_a3) - 1.0);
	double beta_m = cp.e_5_beta_b1 * exp(-x[2] / cp.e_5_beta_b2);
	ks[4] = alpha_m * (1.0 - x[4]) - beta_m * x[4];
	 
	double alpha_h = cp.e_6_alpha_a1 * exp(-x[2] / cp.e_6_alpha_a2);
	double beta_h = cp.e_6_beta_b1 / (exp((cp.e_6_beta_b2 - x[2]) / cp.e_6_beta_b3) + 1.0);
