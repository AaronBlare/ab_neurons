...
f_outs = zeros(f_inv_num, y1_gamma_num);
...
for y1_gamma_id = 1:y1_gamma_num
    ...
    for y2_gamma_id = 1:y2_gamma_num
        ...
        for f_in_inv_id = 1:f_inv_num
            ...
            f_outs(f_in_inv_id, y1_gamma_id) = f_out_avg;
        end
    end
end
...
fig = figure;
for y1_gamma_id = 1:y1_gamma_num
	y1_gamma_curr = y1_gamma_start + (y1_gamma_id - 1) * y1_gamma_shift;
	hName = sprintf('gamma_1 = %0.2f', y1_gamma_curr);
	hLine = plot(f_in_invs, f_outs(:, y1_gamma_id), 'DisplayName', hName);
	hold on
end
set(gca, 'FontSize', 30);
...
savefig(sprintf('%s/f_out_from_f_in_mult_gammas_1_%s.fig', home_figures_path, fn_suffix));

file_name = sprintf('%s/f_out_from_f_in_mult_gammas_1_%s.txt', home_data_path, fn_suffix);
