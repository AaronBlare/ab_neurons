...
for y1_gamma_id = 1:y1_gamma_num
    ...
    for y2_gamma_id = 1:y2_gamma_num
        ...
        for f_inv_id = 1:f_inv_num
            ...
            f_outs(y1_gamma_id) = f_out_curr / seed_num;
        end
    end
end

for y1_id = 1:length(f_outs)
    f_outs(y1_id) = f_outs(y1_id) * 1000;
end

fig = figure;
hLine = plot(y1_gammas, f_outs);
set(gca, 'FontSize', 30);
...
savefig(sprintf('%s/f_out_from_gamma_1_%s.fig', home_figures_path, fn_suffix));

file_name = sprintf('%s/f_out_from_gamma_1_%s.txt', home_data_path, fn_suffix);
file_id = fopen(file_name, 'w');
for y1_id = 1:size(y1_gammas, 1)
    fprintf(file_id, '%0.18e %0.18e\n', y1_gammas(y1_id), f_outs(y1_id));
end
fclose(file_id);