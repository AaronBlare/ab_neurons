...
f_cut_offs = zeros(y1_gamma_num, 1);
...
for y1_gamma_id = 1:y1_gamma_num
    ...
    for y2_gamma_id = 1:y2_gamma_num
        ...
        for f_id = 1:length(f_outs)
            if f_outs(f_id) > 0.1
                f_cut_offs(y1_gamma_id) = f_in_invs(f_id);
                break
            end
        end
    end
...
fig = figure;
hLine = plot(y1_gammas, f_cut_offs);
set(gca, 'FontSize', 30);
...
savefig(sprintf('%s/f_cutoff_from_gamma_1_%s.fig', home_figures_path, fn_suffix));

file_name = sprintf('%s/f_cutoff_from_gamma_1_%s.txt', home_data_path, fn_suffix);
file_id = fopen(file_name, 'w');
for id = 1:length(f_cut_offs)
    fprintf(file_id, '%0.18e %0.18e\n', y1_gammas(id), f_cut_offs(id));
end
fclose(file_id);