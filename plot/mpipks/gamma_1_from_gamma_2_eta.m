...
etas = zeros(y1_gamma_num, y2_gamma_num);
...
for y1_gamma_id = 1:y1_gamma_num
    ...
    for y2_gamma_id = 1:y2_gamma_num
        ...
        etas(y1_gamma_id, y2_gamma_id) = eta_avg;
    end
end

fig = figure;

hLine = imagesc(y1_gammas, y2_gammas, etas);
set(gca, 'FontSize', 30);
...
savefig(sprintf('%s/gamma_1_from_gamma_2_eta_%s.fig', home_figures_path, fn_suffix));

file_name = sprintf('%s/gamma_1_from_gamma_2_eta_%s.txt', home_data_path, fn_suffix);
file_id = fopen(file_name, 'w');
for y1_id = 1:size(y1_gammas, 1)
	for y2_id = 1:size(y2_gammas, 1)
		fprintf(file_id, '%0.18e %0.18e %0.18e\n', y1_gammas(y1_id), y2_gammas(y2_id), etas(y1_id, y2_id));
	end
end
fclose(file_id);