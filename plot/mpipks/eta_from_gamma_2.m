...
for y1_gamma_id = 1:y1_gamma_num
    ...
    for y2_gamma_id = 1:y2_gamma_num
        ...
        etas(y2_gamma_id) = eta_curr / seed_num;
    end
end

fig = figure;
hLine = plot(y2_gammas, etas);
set(gca, 'FontSize', 30);
...
savefig(sprintf('%s/eta_from_gamma_2_%s.fig', home_figures_path, fn_suffix));

file_name = sprintf('%s/eta_from_gamma_2_%s.txt', home_data_path, fn_suffix);
file_id = fopen(file_name, 'w');
for y2_id = 1:size(y2_gammas, 1)
    fprintf(file_id, '%0.18e %0.18e\n', y2_gammas(y2_id), etas(y2_id));
end
fclose(file_id);