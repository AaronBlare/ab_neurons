...
for f_in_inv_id = 1:f_inv_num
    ...
    eta_curr = eta_curr / seed_num;
    
    etas(f_in_inv_id) = eta_curr;
    
end

fig = figure;
hLine = plot(f_in_invs, etas);
set(gca, 'FontSize', 30);
...
savefig(sprintf('%s/eta_from_f_in_2_diff_neu_%s.fig', home_figures_path, fn_suffix));

file_name = sprintf('%s/eta_from_f_in_2_diff_neu_%s.txt', home_data_path, fn_suffix);
file_id = fopen(file_name, 'w');
for id = 1:size(etas, 1)
    fprintf(file_id, '%0.18e %0.18e\n', f_in_invs(id), etas(id));
end
fclose(file_id);